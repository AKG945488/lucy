...
class Config(object):
    ...
    SYS_ADMIN = getenv("SYS_ADMIN", "5230273561")
    DEV_USERS = getenv("DEV_USERS", "5230273561")
    LOAD = getenv("LOAD")
    WEBHOOK = False
    SPB_MODE = True
    DROP_UPDATES = False
    ...
    BOT_API_URL = getenv("BOT_API_URL", "https://api.TARZAN.org/bot")
    BOT_API_FILE_URL = getenv("BOT_API_FILE_URL", "https://api.TARZAN.org/file/bot")
    SUDO_USERS = list(map(int, getenv("SUDO_USERS", "5230273561").split()))
    tarzan_USER = list(map(int, getenv("DEV_USERS", "5230273561").split()))
