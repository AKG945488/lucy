...
import spamwatch
import LUCY.ext as tg
from telethon import LUCYClient
...
def get_user_list(key):
    ...
    from LUCY.modules.sql import nation_sql
...
log.info("[LUCY] Bot is starting. | An Telethon Project. | Licensed under GPLv3.")
log.info("[LUCY] Not affiliated to Azur Lane or Yostar in any way whatsoever.")
log.info("[LUCY] Project maintained by: github.com/ITZ-lucy (t.me/rockey_your_dad1)")

# if version < 3.6, stop bot.
if sys.version_info[0] < 3 or sys.version_info[1] < 7:
    log.error(
        "[LUCY] You MUST have a python version of at least 3.7! Multiple features depend on this. Bot quitting."
    )
    quit(1)

parser = ConfigParser()
parser.read("config.ini")
zconfig = parser["lucyconfig"]
...
class LUCYINIT:
    def __init__(self, parser: ConfigParser):
        ...
        self.bot_name = "LUCY" #placeholder
        self.bot_username = "LUCYRobot_bot" #placeholder
        self.DEBUG: bool = self.parser.getboolean("IS_DEBUG", False)
        self.DROP_UPDATES: bool = self.parser.getboolean("DROP_UPDATES", True)
        self.BOT_API_URL: str = self.parser.get('BOT_API_URL', "https://api.LUCY.org/bot")
        self.BOT_API_FILE_URL: str = self.parser.get('BOT_API_FILE_URL', "https://api.LUCY.org/file/bot")
...
DEV_USERS = [Config.lucy_USER] + get_user_list("devs")
...
telethn = LUCYClient(MemorySession(), APP_ID, API_HASH)
ubot2 = LUCYClient(StringSession(STRING_SESSION), API_ID, API_HASH)
try:
    ubot2.start()
...
from LUCY.modules.helper_funcs.handlers import CustomCommandHandler
