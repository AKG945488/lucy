...
from LUCY import Update, ParseMode, InlineKeyboardMarkup, InlineKeyboardButton
from LUCY.error import (LUCYError, Unauthorized, BadRequest,
                            TimedOut, ChatMigrated, NetworkError)
from LUCY.ext import (
    CallbackContext,
    Filters
)
from LUCY.ext.dispatcher import DispatcherHandlerStop
from LUCY.utils.helpers import escape_markdown
...
from LUCY.modules.helper_funcs.decorators import lucy, lucycallback, lucymsg
...
def send_help(chat_id, text, keyboard=None):
    ...
    if not keyboard:
        ...
        # kb.append([InlineKeyboardButton(text='Support', url='op'),
...
@lucycallback(pattern=r'start_back')
@lucy(command='start', pass_args=True)
                    ...
                    [ 
                        [
                            InlineKeyboardButton(
                                text=gs(chat.id, "add_bot_to_group"),
                        ...
                        [
                            ...
                            InlineKeyboardButton(
                                text=gs(chat.id, "source"),
                                url="https://github.com/",
                            ),
                        ],
                        [
                            InlineKeyboardButton(
                                text=gs(chat.id, "support_chat"),
                                url='kiss',
                            ),
                            InlineKeyboardButton(
                                text="Help",
                            ...
                            InlineKeyboardButton(
                                text=gs(chat.id, "updates_channel"),
                                url="kiss2",
                            ),
                        ],
                    ]
...
def error_callback(_, context: CallbackContext):
    ...
    try:
        ...
        pass
        # handle all other LUCY related errors


@lucycallback(pattern=r'help_')
...
@lucymsg(Filters.status_update.migrate)
...
def main():
    ...
    if WEBHOOK:
        ...
        LUCYINIT.bot_id = dispatcher.bot.id
        LUCYINIT.bot_username = dispatcher.bot.username
        LUCYINIT.bot_name = dispatcher.bot.first_name
...
if __name__ == "__main__":
    log.info("[LUCY] Successfully loaded lucy modules: " + str(ALL_MODULES))
