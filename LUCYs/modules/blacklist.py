import html
import re
from LUCY import ParseMode, ChatPermissions
from LUCY.error import BadRequest
from LUCY.ext import Filters
...
from LUCY import log, dispatcher
from LUCY.modules.helper_funcs.chat_status import user_admin as u_admin, user_not_admin
...
@lucy(command="blacklist", pass_args=True, admin_ok=True)
@u_admin
@typing_action
...
@lucy(command="addblacklist", pass_args=True)
...
@lucy(command="unblacklist", pass_args=True)
@user_admin(AdminPerms.CAN_DELETE_MESSAGES)
@typing_action
...
@lucy(command="blacklistmode", pass_args=True)
@loggable
...
@lucymsg(((Filters.text | Filters.command | Filters.sticker | Filters.photo) & Filters.chat_type.groups), group=BLACKLIST_GROUP)
@user_not_admin
