from LUCY.modules.disable import DisableAbleCommandHandler, DisableAbleMessageHandler
from LUCY.ext import CommandHandler, MessageHandler, CallbackQueryHandler, InlineQueryHandler
from LUCY.ext.filters import BaseFilter
from LUCY import dispatcher as d, log
from typing import Optional, Union, List


class LUCYLUCYHandler:
    def __init__(self, d):
        self._dispatcher = d
    ...
    def command(
        ...
        def _command(func):
            try:
                ...
                log.debug(f"[LUCY] Loaded handler {command} for function {func.__name__} in group {group}")
            except TypeError:
                if can_disable:
                    self._dispatcher.add_handler(
                        DisableAbleCommandHandler(command, func, filters=filters, run_async=run_async,
                ...
                log.debug(f"[LUCY] Loaded handler {command} for function {func.__name__}")
    ...
    def message(self, pattern: Optional[BaseFilter] = None, can_disable: bool = True, run_async: bool = True,
        ...
        def _message(func):
            try:
                ...
                log.debug(f"[LUCY] Loaded filter pattern {pattern} for function {func.__name__} in group {group}")
            except TypeError:
                if can_disable:
                    self._dispatcher.add_handler(
                ...
                log.debug(f"[LUCY] Loaded filter pattern {pattern} for function {func.__name__}")
    ...
    def callbackquery(self, pattern: str = None, run_async: bool = True):
        def _callbackquery(func):
            ...
            log.debug(f'[LUCY] Loaded callbackquery handler with pattern {pattern} for function {func.__name__}')
            return func
    ...
    def inlinequery(self, pattern: Optional[str] = None, run_async: bool = True, pass_user_data: bool = True,
        ...
        def _inlinequery(func):
            ...
            log.debug(
                f'[LUCY] Loaded inlinequery handler with pattern {pattern} for function {func.__name__} | PASSES '
...
lucy = LUCYLUCYHandler(d).command
lucymsg = LUCYLUCYHandler(d).message
lucycallback = LUCYLUCYHandler(d).callbackquery
lucyinline = LUCYLUCYHandler(d).inlinequery
