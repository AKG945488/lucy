...
import LUCY
from LUCY import Chat, ParseMode, InlineKeyboardMarkup, Message, InlineKeyboardButton
...
from LUCY import dispatcher, log, SUDO_USERS
...
from LUCY.modules.helper_funcs.decorators import lucy, lucymsg, lucycallback
...
@typing_action
@lucy(command='filters', admin_ok=True)
def list_handlers(update, context):
    ...
    for keyword in all_handlers:
        ...
        if len(entry) + len(filter_list) > LUCY.MAX_MESSAGE_LENGTH:
            send_message(
                update.effective_message,
                filter_list.format(chat_name),
                parse_mode=LUCY.ParseMode.MARKDOWN,
            )
            filter_list = entry
        else:
            filter_list += entry
...
@lucy(command='filter', run_async=False)
...
# NOT ASYNC BECAUSE DISPATCHER HANDLER RAISED
@lucy(command='stop', run_async=False)
@user_admin(AdminPerms.CAN_CHANGE_INFO)
@typing_action
...
@lucymsg((CustomFilters.has_text & ~Filters.update.edited_message))
                    ...
                    if excp.message == "Unsupported url protocol":
                        try:
                            send_message(
                                ...
                                "LUCY doesn't support buttons for some protocols, such as tg://. Please try "
                                "again...",
                            )
                        except BadRequest as excp:
                            log.exception("Error in filters: " + excp.message)
...
@lucy(command="removeallfilters", filters=Filters.chat_type.groups)
...
@lucycallback(pattern=r"filters_.*")
...
def get_exception(excp, filt, chat):
    if excp.message == "Unsupported url protocol":
        return "You seem to be trying to use the URL protocol which is not supported. LUCY does not support key for multiple protocols, such as tg: //. Please try again!"
    elif excp.message == "Reply message not found":
        return "noreply"
    else:
