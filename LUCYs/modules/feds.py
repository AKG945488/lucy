...
from LUCY.error import BadRequest, LUCYError, Unauthorized
from LUCY import (
    ParseMode,
    Update,
    Chat,
...
from LUCY.utils.helpers import mention_html, mention_markdown
...
from LUCY.modules.helper_funcs.alternate import (
    send_message,
    typing_action,
    send_action,
)
from LUCY.modules.helper_funcs.decorators import lucy, lucycallback
...
log.info("Original federation module by MrYacha, reworked by Mizukito Akito (@peaktogoo) on LUCY.")
...
@typing_action
@lucy(command='delfed', pass_args=True)
...
@typing_action
@lucy(command='chatfed', pass_args=True)
...
@typing_action
@lucy(command='joinfed', pass_args=True)
...
@typing_action
@lucy(command='fpromote', pass_args=True)
...
@typing_action
@lucy(command='fdemote', pass_args=True)
...
@typing_action
@lucy(command='fedinfo', pass_args=True)
...
@typing_action
@lucy(command='fedadmins', pass_args=True)
...
@typing_action
@lucy(command=['fban', 'fedban'], pass_args=True)
def fed_ban(update, context):  # sourcery no-metrics
    ...
    if int(user_id) in (777000, 1087968824):
        message.reply_text("I'm not fbanning LUCY bots.")
        return
...
@typing_action
@lucy(command=['unfban', 'rmfedban'], pass_args=True)
...
@typing_action
@lucy(command='frules', pass_args=True)
...
@typing_action
@lucy(command='fbroadcast', pass_args=True)
...
@lucy(command='fbanlist', pass_args=True, pass_chat_data=True)
...
@typing_action
@lucy(command='fednotif', pass_args=True)
...
@typing_action
@lucy(command='fedchats', pass_args=True)
...
@typing_action
@lucy(command='importfbans', pass_args=True, pass_chat_data=True)
...
@typing_action
@lucy(command='fbanstat', pass_args=True)
...
@typing_action
@lucy(command='setfedlog', pass_args=True)
...
@typing_action
@lucy(command='unsetfedlog', pass_args=True)
...
@typing_action
@lucy(command='unsubfed', pass_args=True)
...
@typing_action
@lucy(command='fedsubs', pass_args=True)
...
@typing_action
@lucy(command='myfeds', pass_args=True)
...
from LUCY.modules.language import gs
...
@lucycallback(pattern=r"fed_help_")
