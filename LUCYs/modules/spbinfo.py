...
import requests, LUCY, datetime, time


@lucy(command="spbinfo")
def lookup(update, context):
    ...
    if response is True:
        ...
        stats += f'*PTID*:\n`{a.get("results").get("private_LUCY_id")}`\n'
        message.reply_text(stats, parse_mode=LUCY.ParseMode.MARKDOWN, disable_web_page_preview=True)
    else:
        message.reply_text("`cannot reach SpamProtection API`", parse_mode=LUCY.ParseMode.MARKDOWN)
        time.sleep(3)
