...
import LUCY.modules.sql.antispam_sql as sql
from LUCY import (
    DEV_USERS,
    GBAN_LOGS,
    OWNER_ID,
...
from LUCY.modules.helper_funcs.extraction import extract_user, extract_user_and_text
from LUCY.modules.helper_funcs.misc import send_to_list
from LUCY import ParseMode, Update
from LUCY.error import BadRequest, LUCYError
...
from  LUCY.modules.helper_funcs.decorators import lucy, lucymsg
...
def gban(update: Update, context: CallbackContext):  # sourcery no-metrics
    ...
    if int(user_id) in (777000, 1087968824):
        message.reply_text("Huh, why would I gban LUCY bots?")
        return
    ...
    if reason:
        if chat.type == chat.SUPERGROUP and chat.username:
            log_message += f'\n<b>Reason:</b> <a href="https://LUCY.me/{chat.username}/{message.message_id}">{reason}</a>'
        else:
...
@lucymsg((Filters.all & Filters.chat_type.groups), can_disable=False, group=GBAN_ENFORCE_GROUP)
